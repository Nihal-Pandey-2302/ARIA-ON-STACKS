...
;; Temporary transfer function allowing marketplace to transfer NFTs
(define-public (transfer
    ...
    (begin
        ;; Allow either the original owner OR the marketplace contract to call transfer
        (let ((current-owner (unwrap-panic (nft-get-owner? rwa-nft token-id))))
            (asserts! (or (is-eq tx-sender sender)
               (is-contract-caller tx-sender)) ; allows the calling contract to transfer
         (err ERR_UNAUTHORIZED))

            ;; Make sure sender actually owns the NFT
